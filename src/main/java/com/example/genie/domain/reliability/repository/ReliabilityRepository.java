...
public interface ReliabilityRepository extends JpaRepository<Reliability, Long> {
    @Query(value = "SELECT * FROM reliability re WHERE re.user_id = :userId ORDER BY re.created_date DESC",
            countQuery = "SELECT count(*) FROM reliability re WHERE re.user_id = :userId",
            nativeQuery = true)
